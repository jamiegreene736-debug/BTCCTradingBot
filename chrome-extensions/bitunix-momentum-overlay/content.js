(() => {
  ...
  function renderContent() {
    ...
    const ftBit = ft && Number.isFinite(ft.count) && ft.count > 0
      ...
      : '';
    status.textContent = payload.error || payload.status?.error || (payload.status?.ready ? '● Monitoring liquid USDT perpetuals' + scanBit + liveBit + ftBit : 'Waiting for complete market data');
    host.querySelector('#bis-planning').innerHTML = `<div><small>Planning equity</small><b>${money(settings.planning_equity)}</b></div><div><small>Risk / trade</small><b>${fixed(settings.risk_pct)}%</b></div><div><small>Leverage / hold</small><b>${settings.leverage}x · ≤${settings.hold_hours}h</b></div><div><small>Profile</small><b>${settings.profile === 'scalp_short' ? 'Scalp short' : 'Swing'}</b></div><button data-action="planning">Edit</button>`;
  ...
  host.addEventListener('click', event => {
    ...
    if (action === 'planning') {
      ...
      const scalp = s.profile === 'scalp_short';
      openForm('Planning settings', `<p>These are planning values, not your exchange balance. Existing tracked plans remain unchanged. <b>Swing</b> scans 4h bias, 1h structure and 15m triggers for a 12h or 24h hold at 25-40x. <b>Scalp short</b> fades parabolic exhaustion on 1m bars for a 1h or 2h hold at up to the pair's cap; the stop sits just above the failed high and must fit inside the estimated liquidation distance, so the card reports the real ceiling. Leverage never tightens a stop.</p><label>Profile<select name="profile" id="bis-profile"><option value="swing" ${scalp ? '' : 'selected'}>Swing · long and short · 12-24h</option><option value="scalp_short" ${scalp ? 'selected' : ''}>Scalp short · 1-2h · high leverage</option></select></label><label>Planning equity (USDT)<input name="planning_equity" type="number" min="10" max="100000000" step="0.01" value="${s.planning_equity}" required></label><label>Risk per trade (%)<input name="risk_pct" type="number" min="0.01" max="2" step="0.01" value="${s.risk_pct}" required></label><label>Leverage (isolated)<input name="leverage" type="number" min="1" max="${scalp ? 125 : 40}" step="1" value="${s.leverage}" required></label><label>Maximum hold<select name="hold_hours">${(scalp ? [1, 2] : [12, 24]).map(h => `<option value="${h}" ${s.hold_hours === h ? 'selected' : ''}>${h} hour${h === 1 ? '' : 's'}</option>`).join('')}</select></label>`, 'Save settings', data => send('save-planning', Object.fromEntries([...data].map(([k, v]) => [k, k === 'profile' ? String(v) : Number(v)]))));
      const profileSelect = host.querySelector('#bis-profile');
      profileSelect?.addEventListener('change', () => {
        const next = profileSelect.value === 'scalp_short';
        const leverage = host.querySelector('.bis-modal [name="leverage"]');
        const hold = host.querySelector('.bis-modal [name="hold_hours"]');
        if (leverage) { leverage.max = next ? 125 : 40; if (Number(leverage.value) > Number(leverage.max)) leverage.value = leverage.max; }
        if (hold) hold.innerHTML = (next ? [1, 2] : [12, 24]).map(h => `<option value="${h}">${h} hour${h === 1 ? '' : 's'}</option>`).join('');
      });
    }
